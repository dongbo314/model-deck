...
| 标签 | 含义 |
|---|---|
| `0.1.0-alpha.7` | 当前预览版的固定版本标签 |
| `alpha` | 始终指向最新 alpha 预览版的浮动标签 |
| `latest` | 有意不发布 |

仓库中的 Compose 文件默认使用 `docker.io/esofk/model-deck:0.1.0-alpha.7`。需要可重复部署时应优先使用固定版本标签；只有在明确希望自动跟随最新预览版时，才使用 `alpha`。
...
控制面板会自行提供 Noto Sans SC 网页字体。升级后如果中文仍显示为带十六进制编码的方框，请先确认运行镜像是 `0.1.0-alpha.7`，再强制刷新页面，并检查 `/_next/static/media/noto-sans-sc-*.woff2` 请求是否成功。给容器安装桌面字体不会改变宿主机浏览器可用的字体。

`0.1.0-alpha.6` 的原生下拉菜单仍可能缺字：Firefox 的原生选项弹窗无法使用网页字体。`0.1.0-alpha.7` 已将语言、模型和角色选择框统一改为页面内列表，展开后的中文选项也使用随应用提供的字体，支持方向键、Enter、空格、Escape、Tab 和按名称前缀定位。浏览器标签栏等浏览器自身界面的字体仍由浏览器和宿主系统控制。
